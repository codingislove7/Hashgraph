...
function createHederaClient() {
  ...
  const operatorPrivateKey = Ed25519PrivateKey.fromString(
...
async function createContract() {
  ...
  const smartContractByteCode =
    smartContract.contracts["stateful.sol:StatefulContract"].bin;
  ...
  const record = await (
    await new ContractCreateTransaction()
      ...
      .setConstructorParams(
        new ContractFunctionParams().addString("hello from hedera")
      )
  ...
  console.log(
    "contract create gas used:",
    record.getContractCreateResult().gasUsed
  );
  console.log(
    "contract create transaction fee:",
    record.transactionFee.asTinybar()
  );
  ...
  // Update the message
  const getRecord = await (
    await new ContractExecuteTransaction()
      .setContractId(newContractId)
      .setGas(7000) // ~6016
      .setFunction(
        "setMessage",
        new ContractFunctionParams().addString("hello from hedera again!")
      )
      .execute(hederaClient)
  )
  ...
  console.log(
    "execute gas used:",
    getRecord.getContractExecuteResult().gasUsed
  );
